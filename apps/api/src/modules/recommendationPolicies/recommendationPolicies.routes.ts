...
interface RecommendationPoliciesRouteDeps {
  getAppByKey: typeof getAppByKey;
  listRecommendationPolicyConfigs: typeof listRecommendationPolicyConfigs;
  upsertRecommendationPolicyConfig: typeof upsertRecommendationPolicyConfig;
  writeOperationLog: typeof writeOperationLog;
}

const defaultDeps: RecommendationPoliciesRouteDeps = {
  getAppByKey,
  listRecommendationPolicyConfigs,
  upsertRecommendationPolicyConfig,
  writeOperationLog
...
export function createRecommendationPoliciesRouter(
  ...
  router.get('/api/recommendation-policies', async (req, res, next) => {
    try {
      ...
      const rows = await deps.listRecommendationPolicyConfigs({
  ...
  router.post('/api/recommendation-policies', async (req, res, next) => {
    try {
      ...
      const row = await deps.upsertRecommendationPolicyConfig({
        app_key: appKey,
        platform,
        engine,
...
export default createRecommendationPoliciesRouter();
