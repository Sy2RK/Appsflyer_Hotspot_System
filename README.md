...
- WebUI 全局 AI 悬浮入口
  ...
  - `roas.get_summary` 专门用于“与 AF 面板 / 简报 / 日报口径对齐”的官方 D7 ROAS 查询
    - `scope=budget`：每日简报 / 预算建议口径
    - `scope=asa`：ASA 简报 / ASA 看板口径
    - 返回结果会显式带 `报告日期` 与 `官方 D7 ROAS 窗口 from ~ to`
...
- 通用投放项 / 广告系列监控与预算建议
  ...
  - `D7 ROAS` 统一按 AF Cohort API `roas` KPI 读取：对报告日 `D` 使用 `D-6` 至 `D`
...
- ASA 关键词专项管理
  ...
  - 独立 ASA 简报与飞书推送
  - `D7 ROAS` 已统一切到 AF Cohort API `roas` KPI；`CPP` 仍按已覆盖购买数据展示
  - `待补齐 / 暂无官方快照 / 真实 0.00` 三种状态会显式区分，不再混用
- 每日简报
  - 通用简报
  - ASA 专项简报
...
- 全系统 `D7 ROAS` 已统一到“AF Cohort API roas KPI + 官方 D7 rolling window”口径
  - `complete`：官方 D7 窗口成本全部被 Cohort ROAS 覆盖
  - `partial`：官方 D7 窗口仍有缺口，但已覆盖成本占比达到可采纳阈值（当前 80%）
  - `pending`：官方 D7 窗口仍有缺口，且覆盖率低于 80%；UI 会显示“待补齐（源数据缺失）”
  - `unavailable`：当前还没有可用于判断的 AF Cohort 官方快照
- ASA 关键词管理页面中 `D7 ROAS` 即使不是全量为 0，也可能因为 AF Cohort 覆盖率尚未达到 80% 而显示“待补齐（源数据缺失）”
...
- 预算建议、ASA 建议、ASA 简报、多维表中的 `D7 ROAS` 已统一为“AF Cohort API roas KPI + 官方 D7 rolling window”口径
- 所有用户可见的 `ROAS / D7 ROAS` 展示统一为百分比；底层仍以 ratio 存储，例如 `1.5` 显示为 `150.00%`
- ROAS 缺口会显示 `待补齐 / 部分可采纳 / 暂无官方快照`，不再把源数据缺失展示成 `0.00`
